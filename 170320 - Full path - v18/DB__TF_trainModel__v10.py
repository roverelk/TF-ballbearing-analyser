...
def array_to_TF_def(tuplet_input):
    ...
    train_feature_batch, train_label_batch, test_feature_batch, test_label_batch, gradient_length, print_intermidiate_values_train, \
    print_intermidiate_values_test, print_error_rate, print_graph, fil_navn, iteration_number, fil_ny = tuplet_input
    ...
    train_size_x = len(train_feature_batch[0]) # Size of X-element
    train_size_y = 1 # Size of Y-element
    #######################
    ...
    train_step = tf.train.AdamOptimizer(gradient_length).minimize(cost)

    #correct_prediction = tf.equal(tf.argmax(y_, 1), tf.argmax(y, 1))
    #accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    accuracy = 100 / tf.reduce_mean(tf.square(y_-y))

    # Initialise all variables
    sess.run(tf.global_variables_initializer())

    # Re-shape for training
    train_feature_batch_shaped = np.asarray(train_feature_batch)
    train_label_batch_shaped = np.asarray(train_label_batch)
    ...
    for i in range(train_iterations):
        # Converting format for test
        train_feature_batch_shaped[i] = np.array(train_feature_batch_shaped[i], np.float32).reshape(1, len(train_feature_batch_shaped[i]))
        train_label_batch_shaped[i] = np.array(train_label_batch_shaped[i], np.float32).reshape(1, 1)

        # Print to system out a status of self evaluation of training
        if i % 10 == 0:
            k_str = set_number_of_zeros(train_feature_batch, i)
            train_accuracy = accuracy.eval(feed_dict={x: train_feature_batch_shaped, y: train_label_batch_shaped, keep_prob: 1.0})
        ...
        # Saving an iteration of the model every 50th training iteration.
        if i % 50 == 0:
            saver.save(sess, saver_file, global_step=(i+iteration_number))

        #The TensorFlow training step
        train_step.run(feed_dict={x: train_feature_batch_shaped, y: train_label_batch_shaped, keep_prob: 0.5})

        # Allows the GUI to run simultaniously as the TensorFlow process is running.
        QApplication.processEvents()

    # Re-shape for testing
    ...
    for i in range(len(test_feature_batch_shaped)):
        ...
        test_label_batch_shaped[i] = np.array(test_label_batch_shaped[i], np.float32).reshape(1, 1)
